...
subroutine del(lat1,lon1,lat2,lon2,dist,baz)

! input: latitudes, longitudes in degree
! dist is epicentral distance (deg) between the two points
! baz is the angle between N and the direction of (lat2,lon2) as
!   seen from (lat1,lon1). It is between -180 and +180, positive over East.
...
if(abs(lat1-lat2)+abs(lon1-lon2)<1.0e-5) then
  dist=0.
  baz=0.
  return
endif  
...
subroutine t2epoch(t0,epoch)
integer, intent(in) :: t0(6)
integer, intent(out) :: epoch
call date2epoch(t0(1),t0(2),t0(3),t0(4),t0(5),epoch)
return
end
...
! we cheat for 1964-1969 (result is negative epoch)
if(year<1970) then
  epoch=0
  db=.false.
  do i=1969,year+1,-1
    epoch=epoch-31536000
    if(i.eq.1968) epoch=epoch-86400
  enddo  
  epoch=epoch-(365-jday)*86400
  if(year.eq.1964.or.year.eq.1968) epoch=epoch-86400
  epoch=epoch-(23-hour)*3600-(59-minut)*60-(60-nsec)
  if(db) write(13,*) 'd2ep:',year,jday,hour,minut,nsec,epoch
  return
endif  
...
function prtm(t)
! generate printable date from t()
character*19 :: prtm
integer, intent(in) :: t(6)
integer :: jj
write(prtm,'(2i4,i3.2,1h:,i2.2,1h:,i2.2,f2.1)') (t(jj),jj=1,5), &
  min(0.001*t(6),0.9)
end function prtm

function prtep(epoch)
! generate printable date from epoch
character*19 :: prtep,prtm
integer, intent(in) :: epoch
integer :: t(6)
call epoch2date(epoch,t(1),t(2),t(3),t(4),t(5))
t(6)=0
prtep=prtm(t)
end function prtep
